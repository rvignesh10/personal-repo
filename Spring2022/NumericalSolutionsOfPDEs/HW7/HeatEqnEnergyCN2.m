function [err,norm_err,xd] = HeatEqnEnergyCN2(N,nStep,tf,fOption,cOption)
...
x = (xlim1+dx/2:dx:xlim2-dx/2);
x = [xlim1-dx/2 x xlim2+dx/2];

NTot = length(x);

assert(length(x)==NTot,'Length mismatch');
...
for i=2:length(t)
    ...
    for j=1:NTot
        if j==1
            b(j) = dx*getUx(xlim1,t(i),fOption);
        elseif j==NTot
