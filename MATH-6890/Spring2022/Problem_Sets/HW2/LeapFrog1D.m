function [] = LeapFrog1D(N,dt,xlim1,xlim2,tlim1,tlim2)
...
% points. Leap Frog time stepping is used for time marching. 
...
% set solution values at time = dt for the spatial grid
u(2, ja:jb)   = sin(2*pi*x)*exp(-nu*4*pi^2*dt);
u(2,ja)       = a;
u(2,jb)       = b;
u(2,1)        = 2*u(2,ja) - u(2,ja+1);
u(2,NP)       = 2*u(2,jb) - u(2,jb-1);

for i=3:length(t)
    for j = ja:jb
        u(i,j) = u(i-2,j) + r*(u(i-1,j+1)-2*u(i-1,j)+ u(i-1,j-1));
    end
...
title('LeapFrog time marching',str,'Interpreter','latex','FontSize',14);
...
title('LeapFrog time marching','Surface Plot');
end
