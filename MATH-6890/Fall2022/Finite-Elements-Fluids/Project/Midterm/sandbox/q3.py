...
def get_xpoints():
    ...
    # xpoints = np.array([0.0,0.125,0.25,0.5,0.75,0.875,1.0])
    return xpoints
...
def apply_num_scheme():
    ...
    for e in range(Ne): # loop index in [0,Ne-1]
        ...
        h = get_h(e)
        tau = get_tau(e) # constant over mesh when ax, kappa, c and h are constants
        ...
        for q in range(neq): # loop index in [0,neq-1]
            ...
            for idx_a in range(nes): # loop index in [0,nes-1]
                be[idx_a] = be[idx_a] + shp[idx_a,q]*s*wdetj -c*f_stab*tau*s*shp[idx_a,q]*wdetj + ax*tau*shpdgbl[idx_a,q]*s*wdetj#  source term
