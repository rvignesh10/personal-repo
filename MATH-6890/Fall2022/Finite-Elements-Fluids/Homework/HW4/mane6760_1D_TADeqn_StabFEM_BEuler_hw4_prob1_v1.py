...
def get_tmax():
    # return max time
    tmax = 0.25*get_L()/get_ax() # time to travel 25% of the spatial domain with velocity ax
    return tmax
...
def get_Nt():
    ...
    Nt = 250 # note number of steps is Nt+1 including t_0 for IC
    return Nt
...
def get_IC():
    ...
    Nnleft = int(Nn/4.0) # Nnleft spans the first 25% of the domain (left side)
    for i in range(Nnleft,Nn): # loop index in [Nnleft,Nn-1]
        phi_sfem[i] = 1.0
    return phi_sfem
...
def apply_num_scheme():
    ...
    for n in range(1,Nt+1): # loop index [1,Nt]
        ...
        for e in range(Ne): # loop index in [0,Ne-1]
            ...
            for q in range(neq): # loop index in [0,neq-1]
                ...
                for idx_a in range(nes): # loop index in [0,nes-1]
                    be[idx_a] = 0.0
                    # de[idx_a] = de[idx_a] +  \
                    #             phiq*shp[idx_a,q]*wdetj + \
                    #             phiq*shpdgbl[idx_a,q]*tau*ax*wdetj # ... to be implemented ...
        ...
        # print(phi_sfem) # debug
    s = 'q1_'+'Nt_'+str(Nt)+'.pdf'
    if (display_phi_plot):
        plt.plot(xpoints,phi_sfem,'ro-',label='sol. at final step')
