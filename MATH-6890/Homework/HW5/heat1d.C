...
// ------------------------------------------------------------------------
int writeMatlabVector(FILE *matlabFile, Real *u_p, const char *name, int nd1a, int nd1b, int nThreads){
    ...
    int i;
    #pragma omp parallel num_threads(nThreads) shared(u_p, nd1a, nd1b) private(i)
    {
      #pragma omp for
      for ( i=nd1a; i<=nd1b; i++ )
      {
...
int main(int argc, char *argv[]){
    ...
    int numThreads = 4;   // default num of parallel threads
    ...
    int i;
    #pragma omp parallel num_threads(numThreads) shared(x_p, xa, dx) private(i)
    {
      #pragma omp for
      for (i=nd1a; i<=nd1b; i++) {
        x(i) = xa + (i - ja) * dx;
    ...
    int id;
#pragma omp parallel num_threads(numThreads) shared(u_p, x_p, curr, t) private(id)
    {
      #pragma omp for
      for (id = nd1a; id<=nd1b; id++ )
      {
	uc(id) = UTRUE( x(id), t );
      }
    }
    ...
    for (int n=0; n<numSteps; n++) {
        ...
        int j;
          ...
          #pragma omp parallel num_threads(numThreads) shared(u_p, x_p, t, dt) private(j)
	  {
	    #pragma omp for
	    for ( j=ja; j<=jb; j++ )
	    {
	  ...
	  // write RK2 solver
            ...
            int k;
            #pragma omp parallel num_threads(numThreads) shared(u_p, x_p, w_p, t, dt) private(k)
	    {
	      #pragma omp for
	      for( k=ja; k<=jb; k++ ){
		w(k) = uc(k) + (rx/2.0) * (uc(k+1) - 2.*uc(k) + uc(k-1)) + 0.5 * dt * FORCE( x(k), t );
	      }
	    }
            ...
            #pragma omp parallel num_threads(numThreads) shared(u_p, w_p, x_p, t, dt) private(k)
	    {
	      #pragma omp for
	      for( k=ja; k<=jb; k++ ){
		un(k) = uc(k) + rx * (w(k+1) - 2.*w(k) + w(k-1)) + dt * FORCE( x(k), t+0.5*dt );
	      }
	    }
            //for (int i=ja; i<=jb; i++) {
    ...
    Real maxErr = 0.0;
    int m;
    #pragma omp parallel num_threads(numThreads) shared(u_p, error_p, x_p, t, maxErr) private(m)
    {
      #pragma omp for
      for ( m=nd1a; m<=nd1b; m++ ){
	error(m) = uc(m) - UTRUE( x(m), t );
	maxErr = max( maxErr, abs(error(m)) );
      }
    }
    //for (int i=nd1a; i<=nd1b; i++) {
...
// --- Write a file for plotting in matlab ---
    ...
    writeMatlabVector( matlabFile, x_p, "x", nd1a, nd1b, numThreads );
    writeMatlabVector( matlabFile, u_p[curr], "u", nd1a, nd1b, numThreads );
    writeMatlabVector( matlabFile, error_p, "err", nd1a, nd1b, numThreads );
