...
extern "C"
{
  void heat2dUpdate( const int &n1a, const int &n1b, const int &n2a, const int &n2b,
		     ...
		     Real &un, const Real &u, const Real &rx, const Real &ry, int &nThreads );
}
...
int main( int argc, char *argv[] )
{
  ...
  int numThreads=4; // default num of threads to use
  ...
  //{
#pragma omp parallel num_threads(numThreads) shared(x, dx, xa, ya) private(i1,i2)
    {
#pragma omp for collapse(2)
  ...
  #pragma omp parallel num_threads(numThreads) shared(u0, x, t) private(i1, i2)
  {
    #pragma omp for collapse(2)
    for( i2=nd2a; i2<=nd2b; i2++ )
    for( i1=nd1a; i1<=nd1b; i1++ )
  ...
  for( n=0; n<numSteps; n++ )
  {
    ...
    {
      #pragma omp parallel num_threads(numThreads) shared(u, un, rx, ry) private(i1, i2)
      {
        #pragma omp for collapse(2)
    ...
    else if( option==cIndexing )
    {
      ...
      //for( i2=n2a; i2<=n2b; i2++ )
	//{
	//#pragma omp parallel num_threads(numThreads) shared(u_p, un_p, rx, ry) private(i1)
	//{
	  //#pragma omp for
	  //for( i1=n1a; i1<=n1b; i1++ )
	    // {
	  ...
	  //}
	//}
      #pragma omp parallel num_threads(numThreads) shared(u_p, un_p, rx, ry) private(i1, i2)
      {
        #pragma omp for collapse(2)
	for( i2=n2a; i2<=n2b; i2++ )
	for( i1=n1a; i1<=n1b; i1++ )
	...
	const double *u_p = u.getDataPointer();
	double *un_p = un.getDataPointer();
	heat2dUpdate( n1a, n1b, n2a, n2b, u.getBase(0), u.getBound(0), u.getBase(1), u.getBound(1), *un_p, *u_p, rx, ry, numThreads);
      }
	...
	{ // left or right side
	  ...
	  int i1g = i1 - is; // index of ghost point
          #pragma omp parallel num_threads(numThreads) shared(un, u, x, t, dt, i1, i1g, is) private(i2)
	  {
	    #pragma omp for
	    for( i2=nd2a; i2<=nd2b; i2++ )
	    {
	...
	{ // bottom or top
	  ...
	  int i2g = i2 - is; // index of ghost point
          #pragma omp parallel num_threads(numThreads) shared(u, un, x, t, dt, i2, i2g, is) private(i1)
	  {
	   #pragma omp for
	    for( i1=nd1a; i1<=nd1b; i1++ )
	    {
  ...
  #pragma omp parallel num_threads(numThreads) shared(uc, err, x, tFinal, maxErr, maxNorm) private(i1, i2)
  {
    #pragma omp for collapse(2)
    for( i2=n2a; i2<=n2b; i2++ )
    for( i1=n1a; i1<=n1b; i1++ )
  ...
  if( saveMatlab )
  {
    ...
    if( saveMatlab>1 )
    {
      writeMatlabArray( matlabFile, x, "x", 2, dimension, numThreads );
      writeMatlabArray( matlabFile, ua[cur], "u", 1, dimension, numThreads );
      writeMatlabArray( matlabFile, err, "err", 1, dimension, numThreads );
    }
    fclose(matlabFile);
